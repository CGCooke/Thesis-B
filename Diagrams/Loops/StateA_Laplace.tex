...
\begin{tikzpicture}[auto, node distance=2cm,>=latex']
    ...
    %FLL Stuff
    \node [block, right of=Rotator,node distance=3cm] (DiscriminatorFLL) {$\frac{d}{dt}$};
    ...
    \node [block, below of = Rotator](VCO){$\displaystyle \int$};
    ...
    \node [block, below of = VCOGain](VCOIntegrator){$\displaystyle \int$};
    ...
    \draw [-] (SamplingTime) |- node{$f_{Error}$} (K1FLL);
